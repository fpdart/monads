...
nothing() {
  test('Nothing all', () {
    ...
    expect(a.getOrElse<int>(123) == new Just(123), true);

    expect(Nothing().orJust<int>(2) == new Just<int>(2), true);
  });
}

just() {
  test('Create object and get the value', () {
    var a = new Just<int>(123);

    expect(a.just, 123);
    expect(a.isNothing, false);
    expect(Just(Nothing()).isNothing, true);

    expect(new Just<Nothing>(new Nothing()).isNothing, true);
    expect(new Just<Null>(null).isNothing, true);
  });

  test('Map just', () {
    var c = new Just<List<int>>([5, 4, 1, 4, 3]);
    var d = new Just<List<int>>([1, 2, 3]);
  ...
  test('Fold just', () {
    var a = new Just<int>(123);
    var b = new Just<Null>(null);
    var c = new Just<List>([]);
    var d = new Just<Nothing>(new Nothing());

    expect(a.fold<int, int>(1)((v) => v + 1), 124);
    expect(b.fold<String, String>('ha')((v) => v + 1), 'ha');
    expect(d.fold<int, int>(1)((v) => v), 1);
    expect(
        c.fold<int, List<dynamic>>(123)((v) {
          v.add(1);
